...
export default function LiveMatches({
  ...
  const [filter, setFilter] = useState<"all" | "live" | "today" | "finished">(
    homeMode ? "today" : "all",
  );
  ...
  const MatchTile = ({ m }: { m: MatchData }) => {
    ...
    const dbMatch = ALL_MATCHES.find(
      (g) => g.home_team === m.homeTeam.tla && g.away_team === m.awayTeam.tla,
    );
    ...
    if (
      ...
      const isKo = ["R32", "R16", "QF", "SF", "3RD", "FINAL"].includes(
        dbMatch.group_label,
      );
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          >
            ...
            <span
              style={{ fontSize: 13, fontWeight: 600, color: "var(--cream)" }}
            >
        ...
        {(m.goals?.length ?? 0) > 0 && m.goals && (
          ...
          >
            ...
            <span style={{ opacity: 0.6 }}>
              ({m.goals[m.goals.length - 1].team.name.slice(0, 3).toUpperCase()}
              )
            </span>
          </div>
        )}
      ...
      {selectedMatch && (
        <MatchModal
          ...
          localMatchId={
            ALL_MATCHES.find(
              (g) =>
                g.home_team === selectedMatch.homeTeam.tla &&
                g.away_team === selectedMatch.awayTeam.tla,
            )?.id
          }
          prediction={
            predictions.find(
