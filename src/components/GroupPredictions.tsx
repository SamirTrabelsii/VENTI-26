...
const GroupPredictions = memo(function GroupPredictions({
  activeMatches,
  userId,
  nextGroup,
}: Props) {
  ...
  const standings = () => {
    ...
    activeMatches.forEach((m) => {
      if (!teams[m.home_team])
        teams[m.home_team] = { p: 0, w: 0, d: 0, l: 0, gf: 0, ga: 0, pts: 0 };
      if (!teams[m.away_team])
        teams[m.away_team] = { p: 0, w: 0, d: 0, l: 0, gf: 0, ga: 0, pts: 0 };
    });
          ...
          <tbody>
            {table.map((row, i) => (
              ...
              >
                ...
                >
                  <span style={{ color: "var(--muted)", width: 14 }}>
                    {i + 1}
                  </span>
