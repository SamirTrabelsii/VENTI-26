// src/components/ScoringRulesDrawer.tsx
'use client'
                        ...
                        <div className="flex-1 overflow-y-auto pb-10" style={{ scrollbarWidth: 'thin' }}>
                            ...
                            <div className="p-6 border-b border-[var(--border)] relative overflow-hidden">
                                ...
                                <p className="text-[11px] text-dim leading-relaxed mb-5">
                                    Same rules apply for group stage and knockout matches.
                                </p>

                                <div className="flex flex-col gap-4 relative z-10">
                                    {SCORING_REFERENCE.groupAndKnockout.map((r) => (
                                        <div key={r.label} className="flex items-start gap-4">
                                            ...
                                            >
                                                <span className={`font-display text-xl ${typeof r.pts === 'number' && r.pts >= 25 ? 'text-gold' : 'text-cream'}`}>
                                                    +{r.pts}
                                                </span>
                                            </div>
                                            <div className="pt-1">
                                    ...
                                    {SCORING_REFERENCE.knockoutSupplement.map((r) => (
                                        <div key={r.label} className="flex items-start gap-4">
                                            <div className="flex-shrink-0 w-12 h-12 rounded-xl flex items-center justify-center bg-[#e05c4a]/10 border border-[#e05c4a]/20">
                                                <span className="font-display text-xl text-[#e05c4a]">+{r.pts}</span>
                                            </div>
                                            <div className="pt-1">
                                                <div className="text-sm font-semibold text-cream">{r.label}</div>
                                                <div className="text-[11px] text-muted mt-1 leading-relaxed">{r.note}</div>
                            ...
                            {/* Section 3 — Maximum per match */}
                            <div className="p-6 border-b border-[var(--border)] bg-black/20">
                                <h3 className="text-[10px] font-bold tracking-[2px] uppercase text-[var(--gold)] mb-3 flex items-center gap-2">
                                    <span className="w-1.5 h-1.5 rounded-full bg-[var(--gold)]" /> Maximum Per Match
                                </h3>
                                <div className="grid grid-cols-2 gap-3 mt-4">
                                    <div className="flex flex-col items-center justify-center p-4 rounded-xl bg-[var(--surface2)] border border-[var(--border)]">
                                        <span className="font-display text-3xl text-cream">25</span>
                                        <span className="text-[10px] font-bold text-muted mt-1 uppercase tracking-wider">Group Stage</span>
                                        <span className="text-[10px] text-muted mt-1">Exact score</span>
                                    </div>
                                    <div className="flex flex-col items-center justify-center p-4 rounded-xl bg-[var(--surface2)] border border-[var(--border-gold)]"
                                        style={{ background: 'linear-gradient(135deg, rgba(212,168,67,0.08), transparent)' }}>
                                        <span className="font-display text-3xl text-gold">35</span>
                                        <span className="text-[10px] font-bold text-muted mt-1 uppercase tracking-wider">Knockout</span>
                                        <span className="text-[10px] text-muted mt-1">Exact + qualifier</span>
                                    </div>
                                </div>
                            </div>

                            {/* Section 4 — Streaks note */}
                            <div className="p-6">
                                <h3 className="text-[10px] font-bold tracking-[2px] uppercase text-[#5b9fff] mb-3 flex items-center gap-2">
                                    <span className="w-1.5 h-1.5 rounded-full bg-[#5b9fff]" /> Streaks
                                </h3>
                                <p className="text-[11px] text-dim leading-relaxed">
                                    Your streak counts consecutive matches where you predicted the correct result (exact score or correct outcome). A missed prediction or wrong result resets it to zero.
                                </p>
                            </div>
