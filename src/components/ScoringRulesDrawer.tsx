...
export default function ScoringRulesDrawer({
  customTrigger,
}: {
          ...
          >
            ...
            <div className="flex-shrink-0 flex items-center justify-between px-6 py-5 border-b border-[var(--border)] bg-black/20 backdrop-blur-md sticky top-0 z-10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="font-display text-2xl text-cream leading-none tracking-wide">
                    Points Formula
                  </h2>
            ...
            >
                ...
                <div className="flex flex-col gap-4 relative z-10">
                  {SCORING_REFERENCE.groupAndKnockout.map((r) => (
                    <div key={r.label} className="flex items-start gap-4">
                      ...
                      >
                        <span
                          className={`font-display text-xl ${typeof r.pts === "number" && r.pts >= 25 ? "text-gold" : "text-cream"}`}
                        >
                          +{r.pts}
                        </span>
                      </div>
                  ...
                  {SCORING_REFERENCE.knockoutSupplement.map((r) => (
                    <div key={r.label} className="flex items-start gap-4">
                      <div className="flex-shrink-0 w-12 h-12 rounded-xl flex items-center justify-center bg-[#e05c4a]/10 border border-[#e05c4a]/20">
                        <span className="font-display text-xl text-[#e05c4a]">
                          +{r.pts}
                        </span>
                      </div>
                      <div className="pt-1">
              ...
              <div className="p-6 border-b border-[var(--border)] bg-black/20">
                ...
                <div className="grid grid-cols-2 gap-3 mt-4">
                  ...
                  <div
                    className="flex flex-col items-center justify-center p-4 rounded-xl bg-[var(--surface2)] border border-[var(--border-gold)]"
                    style={{
                      background:
              ...
              <div className="p-6">
                <h3 className="text-[10px] font-bold tracking-[2px] uppercase text-[#5b9fff] mb-3 flex items-center gap-2">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#5b9fff]" />{" "}
                  Streaks
                </h3>
                <p className="text-[11px] text-dim leading-relaxed">
                  Your streak counts consecutive matches where you predicted the
                  correct result (exact score or correct outcome). A missed
                  prediction or wrong result resets it to zero.
                </p>
              </div>
            </div>
          </div>
