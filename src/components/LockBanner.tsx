...
export default function LockBanner() {
    ...
    if (phase === 'PRE_TOURNAMENT') {
        message = 'Original predictions open. All group scores and original bracket picks lock at opening kickoff: ' + timeLeft
        bgColor = 'rgba(212, 168, 67, 0.1)'
        borderColor = 'var(--gold)'
        icon = 'open'
    } else if (phase === 'KNOCKOUT_OPEN') {
        message = 'Original predictions are locked. Live knockout predictions open match-by-match as soon as each real fixture is known.'
        bgColor = 'rgba(212, 168, 67, 0.1)'
        borderColor = 'var(--gold)'
        icon = 'live'
    } else {
    ...
    return (
        <div style={{
            ...
            <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', color: 'var(--gold)', minWidth: 46 }}>
                {icon}
            </span>
            <span>{message}</span>
        </div>
