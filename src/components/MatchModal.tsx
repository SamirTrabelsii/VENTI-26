...
export interface MatchData {
  ...
  utcDate: string;
  status:
    "SCHEDULED" | "IN_PLAY" | "PAUSED" | "FINISHED" | "POSTPONED" | string;
  minute?: number;
  stage: string;
  group?: string;
...
const getStatusStyle = (status: string) => {
  switch (status) {
    ...
    case "PAUSED":
      return {
        bg: "rgba(200,57,43,0.15)",
        color: "#e05c4a",
        border: "rgba(200,57,43,0.3)",
        label: "LIVE",
      };
    case "FINISHED":
      return {
...
export default function MatchModal({
  ...
  const homeGoals = safeGoals.filter(
    (g) => g.team.name === match.homeTeam.name,
  );
  const awayGoals = safeGoals.filter(
    (g) => g.team.name === match.awayTeam.name,
  );
    ...
    return leaderboard
      .map((u) => {
        ...
        if (
          ...
          const isKoMatch = dbMatch
            ? ["R32", "R16", "QF", "SF", "3RD", "FINAL"].includes(
                dbMatch.group_label,
              )
            : false;
            ...
            <div style={{ textAlign: "center", padding: "0 16px" }}>
              {isLive || isFinished ? (
                <>
                  ...
                  >
                    <span
                      style={{ color: isLive ? "var(--gold)" : "var(--cream)" }}
                    >
                      {hScore}
                    </span>
                    <span style={{ color: "var(--muted)", fontSize: 40 }}>
                      –
                    </span>
                    <span
                      style={{ color: isLive ? "var(--gold)" : "var(--cream)" }}
              ...
              <div style={{ flex: 1 }}>
                {homeGoals.map((g, i) => (
                  ...
                  >
                    ...
                    {g.type === "OWN_GOAL" && (
                      <span style={{ fontSize: 10, color: "#e05c4a" }}>OG</span>
                    )}
                    {g.type === "PENALTY" && (
              ...
              >
                {displayLeaderboard.map((u, i) => (
                  ...
                  >
                    ...
                    >
                      {u.predicted_home !== null &&
                      u.predicted_away !== null ? (
                        `${u.predicted_home} - ${u.predicted_away}`
                      ) : u.team_code ? (
                        <span
                          style={{ fontSize: 12, textTransform: "uppercase" }}
                        >
                          {u.team_code} advances
                        </span>
                      ) : (
                        "-"
                      )}
