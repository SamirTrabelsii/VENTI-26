...
export default function WhatsNewCarousel({ isGuest }: { isGuest?: boolean }) {
  ...
  const markSeen = () => {
    ...
    if (userId) {
      void supabase.from("user_app_release_views").upsert(
        {
          user_id: userId,
            ...
            <div className="relative p-5 md:p-6">
              <div className="mb-6 flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-[0.24em] text-[#D4A843]">
                      Leaderboard update
                    </p>
                  ...
                  >
                    <span
                      ...
                      style={{
                        ...
                        backgroundColor:
                          index <= active ? item.accent : "transparent",
                      }}
                    />
                  </button>
                ))}
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  <div className="mb-4 flex items-center gap-3">
                    <div
                      className="grid h-12 w-12 place-items-center border bg-black/45"
                    ...
                    <h2
                      id="whats-new-title"
                      className="font-display text-2xl uppercase leading-tight tracking-wide text-white md:text-3xl"
                    >
                      {slide.title}
                    </h2>
                  </div>
                    ...
                    {slide.variant === "leaderboard" && (
                      <div className="space-y-2 border border-[#D4A843]/30 bg-[#D4A843]/[0.06] p-4">
                        <div className="flex items-center justify-between gap-4">
                          <p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#D4A843]">
                            Group stage ranking
                          </p>
                        ...
                        {leadersLoading ? (
                          ...
                          leaders.map((user, index) => {
                            ...
                            return (
                              <div
                                key={user.id}
                                className="flex items-center gap-3 border border-white/10 bg-black/35 p-2"
                              >
                                <div
                                  className="w-7 text-center font-display text-2xl leading-none"
                                  style={{ color }}
                                >
                                  {index + 1}
                                </div>
                                <img
                                  src={getRobohashUrl(user.display_name, 52)}
                          ...
                          <p className="py-4 text-sm text-zinc-400">
                            No completed group-stage scores yet.
                          </p>
                        )}
                      </div>
                    )}
                    ...
                    {slide.variant === "tabs" && (
                      <div className="flex flex-wrap gap-2">
                        {RANKING_TABS.map((tab) => (
                          <div
                            key={tab}
                            className="border border-white/10 bg-white/[0.04] px-3 py-2 text-[10px] font-black uppercase tracking-[0.14em] text-white"
                          >
                            {tab}
                          </div>
                        ))}
