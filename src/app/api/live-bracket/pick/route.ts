...
export async function POST(request: Request) {
  ...
  if (!user)
    return badRequest(
      "You must sign in to save a live bracket prediction.",
      401,
    );
  ...
  const staticFixture = KNOCKOUT_MATCHES.find(
  ...
  const dbMatches = await supabase
    .from("matches")
    .select(
      "id, home_team, away_team, home_score, away_score, penalty_home_score, penalty_away_score, went_to_penalties, kickoff, status, qualifier",
    );
  ...
  const fixture = buildLiveBracketFixtures(
    apiMatches,
    dbMatches.data || [],
  ).find((match) => match.id === fixtureId);
  ...
  const validAdvancingTeam =
    advancingTeam === fixture.homeCode || advancingTeam === fixture.awayCode;
