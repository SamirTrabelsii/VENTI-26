...
export async function GET(
  ...
  const isKnockout = KNOCKOUT_MATCHES.some(
    (m) => m.id.toLowerCase() === id.toLowerCase(),
  );
  ...
  const { data: dbMatch } = await supabase
    .from("matches")
    .select(
      "home_team, away_team, home_score, away_score, status, qualifier, went_to_penalties, penalty_home_score, penalty_away_score",
    )
    .eq("id", localMatch.id)
    .single();
    ...
    try {
      ...
      if (liveRes.ok) {
        ...
        const liveMatch = liveMatches.find(
          (m) =>
            (m._homeCode === dbMatch?.home_team &&
              m._awayCode === dbMatch?.away_team) ||
            (NAME_TO_CODE[m.homeTeam?.name] === dbMatch?.home_team &&
              NAME_TO_CODE[m.awayTeam?.name] === dbMatch?.away_team),
        );
  ...
  if (isKnockout) {
    ...
    if (!slot)
      return NextResponse.json(
        { error: "Unsupported knockout match id" },
        { status: 400 },
      );
    ...
    const bracketPicks = await fetchAllRows(
      supabase
        .from("live_ko_picks")
        .select(
          "home_score, away_score, team_code, predicted_home_team, predicted_away_team, user_id",
        )
        .eq("round", slot.round)
      ...
      const scoreResult = hasScore
        ? scoreMatch(
            ...
            true,
            {
              ...
              realQualifier:
                effectiveQualifier ??
                inferQualifier(dbMatch, effectiveHomeScore, effectiveAwayScore),
            },
          )
        : null;
    ...
    return NextResponse.json({
      ...
      liveScore: hasScore
        ? { home: effectiveHomeScore, away: effectiveAwayScore }
        : null,
  ...
  const predictions = await fetchAllRows(
    supabase
      .from("predictions")
      .select(
        "home_score, away_score, user_id, profile:profiles(display_name, avatar_initials, avatar_color)",
      )
      .eq("match_id", id),
  );
