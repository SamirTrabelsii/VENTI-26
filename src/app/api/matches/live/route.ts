...
export async function GET() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      const matches = (data.matches || []).map((m: any) => {
        const homeCode =
          NAME_TO_CODE[m.homeTeam?.name] || m.homeTeam?.tla || "TBD";
        const awayCode =
          NAME_TO_CODE[m.awayTeam?.name] || m.awayTeam?.tla || "TBD";
    ...
    if (res.ok) {
      ...
      const matches = games.map((g: any) => {
        ...
        return {
          ...
          homeTeam: {
            name: g.home_team_name_en || "TBD",
            shortName: homeCode,
            tla: homeCode,
            crest: "",
          },
          awayTeam: {
            name: g.away_team_name_en || "TBD",
            shortName: awayCode,
            tla: awayCode,
            crest: "",
          },
  ...
  return NextResponse.json(
    { error: "All APIs unavailable", matches: [] },
    { status: 502 },
  );
}
