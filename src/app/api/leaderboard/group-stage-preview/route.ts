...
export async function GET() {
  try {
    ...
    const [profiles, predictions, matches] = await Promise.all([
      ...
      fetchAllRows(
        supabase
          ...
          .in("match_id", GROUP_MATCH_ID_LIST),
      ),
      fetchAllRows(
        supabase
          .from("matches")
          .select(
            "id, group_label, stage, home_score, away_score, status, kickoff",
          )
          .in("id", GROUP_MATCH_ID_LIST),
      ),
    ]);
    ...
    const leaders = profiles
      ...
      .filter(
        (user: any) => user.total_points > 0 || finishedGroupMatches.length > 0,
      )
      .sort(
        (a: any, b: any) =>
    ...
    return NextResponse.json(
      { leaders: [], finished_count: 0, match_count: GROUP_MATCH_TOTAL },
      { status: 500 },
    );
  }
}
