...
export async function POST(request: Request) {
  ...
  const realQualifier: string | null = isKnockout
    ? inferQualifier(match)
    : null;
  ...
  const slot_index = isKnockout
    ? roundSlotFromFixtureId(match.id).slotIndex
    : -1;

  // ── 3. Score every prediction ──────────────────────────────────────────────
  const results = Array.from(allUserIds)
    .map((userId) => {
      ...
      if (isKnockout) {
        const pick = liveKoPicks.find(
          (bp) => bp.user_id === userId && bp.slot_index === slot_index,
        );
  ...
  const { recalculateAllUsers } =
    await import("@/app/api/admin/recalculate/route");
  const recalcResult = await recalculateAllUsers(
    isKnockout ? undefined : userIds,
  );
...
export async function GET(request: Request) {
  ...
  const predsData = await fetchAllRows(
    supabase
      .from("predictions")
      .select(
        "user_id, home_score, away_score, qualifier_pick, profile:profiles(display_name)",
      )
      .eq("match_id", match_id),
  );
