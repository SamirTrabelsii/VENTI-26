...
export async function recalculateAllUsers(userIdFilter?: string[]): Promise<{
    ...
    const userTotals = new Map<string, { total_points: number; exact_scores: number; correct_results: number; streak: number }>()
...
export async function POST(request: Request) {
    const isInternal = request.headers.get('x-scoring-secret') === process.env.SCORING_SECRET
    if (!isInternal) {
