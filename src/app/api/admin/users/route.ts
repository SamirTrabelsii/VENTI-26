...
export async function GET() {
  ...
  const { data: profiles } = await db
    .from("profiles")
    .select(
      "id, email, display_name, avatar_initials, avatar_color, created_at, is_unlocked",
    )
  ...
  allBracket?.forEach((p) =>
    bracketCounts.set(p.user_id, (bracketCounts.get(p.user_id) ?? 0) + 1),
  );
...
export async function POST(request: Request) {
  ...
  if (!["reset", "reset_password"].includes(action) || !user_id) {
    return NextResponse.json(
      { error: "Invalid action or missing user_id" },
      { status: 400 },
    );
  }
