...
export default function AdminPage() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--black)" }}>
      ...
      <div
        style={{ maxWidth: 1400, margin: "0 auto", padding: "88px 32px 60px" }}
      >
        {/* Header */}
        <div style={{ marginBottom: 28 }}>
          ...
          <p style={{ color: "var(--muted)", fontSize: 14, marginTop: 6 }}>
            Platform management, match scoring, and test tools
          </p>
        </div>
...
function OverviewTab() {
  ...
  const statCards = [
    ...
    {
      label: "Users Completed 72",
      value: stats.usersCompleted,
      accent: "var(--green-bright)",
      icon: "🎖️",
    },
  ];
      ...
      <div style={card}>
        ...
        <div style={{ padding: "16px 18px", display: "flex", gap: 20 }}>
          {[
            {
              label: "Upcoming",
              count: stats.matches.upcoming,
              color: "var(--muted)",
            },
            {
              label: "Live",
              count: stats.matches.live,
              color: "var(--green-bright)",
            },
            {
              label: "Finished",
              count: stats.matches.finished,
              color: "var(--gold)",
            },
          ].map((s) => (
            <div
            ...
            >
              ...
              <span style={{ fontSize: 13, color: "var(--dim)" }}>
                {s.label}:{" "}
                <strong style={{ color: "var(--cream)" }}>{s.count}</strong>
              </span>
            </div>
          ))}
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
        ...
        <div style={card}>
          ...
          {stats.recentUsers.map((u) => (
            ...
            >
              <img
                src={getRobohashUrl(u.display_name, 40)}
                alt=""
...
function MatchesTab() {
  ...
  const finishMatch = async (matchId: string) => {
    ...
    setActionLog((prev) => [
      `✅ Match ${matchId} → ${home}-${away} (finished)`,
      ...prev,
    ]);
    ...
    setActionLog((prev) => [
      `📊 Scored ${matchId}: ${scoreResult.processed ?? 0} entries, ${scoreResult.predictions_found ?? 0} predictions`,
      ...prev,
    ]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      {actionLog.length > 0 && (
        <div
          style={{ ...card, padding: 16, maxHeight: 160, overflowY: "auto" }}
        >
          <div
            style={{
              display: "flex",
          ...
          >
            ...
            <button onClick={() => setActionLog([])} style={btnSmall}>
              Clear
            </button>
          </div>
          {actionLog.map((msg, i) => (
                ...
                <tr key={m.id}>
                  ...
                  <td
                    style={{
                      ...
                      fontSize: 20,
                      color:
                        m.home_score !== null ? "var(--cream)" : "var(--muted)",
                    }}
                  >
                    {m.home_score !== null
                  ...
                  <td style={td}>
                    {m.status !== "finished" ? (
                      ...
                      <span
                        style={{ fontSize: 11, color: "var(--green-bright)" }}
                      >
                        ✓ Done
                      </span>
                    )}
                    ...
                    {m.status !== "finished" ? (
                      ...
                      <button
                        onClick={async () => {
                          ...
                          setActionLog((prev) => [
                            `🔄 Re-scored ${m.id}: ${data.processed ?? 0} entries`,
                            ...prev,
                          ]);
                        }}
                        style={btnSmall}
...
function UsersTab() {
  ...
  const resetUser = async (userId: string, name: string) => {
    if (
      !confirm(
        `Reset all predictions, bracket picks, and scores for "${name}"?\n\nThis cannot be undone.`,
      )
    )
      return;
    const res = await fetch("/api/admin/users", {
  ...
  const resetPassword = async (userId: string, name: string) => {
    if (
      !confirm(
        `Generate a temporary password for "${name}"?\n\nTheir old password will stop working immediately.`,
      )
    )
      return;
    const res = await fetch("/api/admin/users", {
    ...
    if (res.ok) {
      setActionMsg(
        `Password reset for ${name}. Temporary password: ${data.temporary_password}`,
      );
    } else {
  ...
  const toggleUnlock = async (
    ...
    if (!confirm(`${newStatus ? "Unlock" : "Lock"} predictions for "${name}"?`))
      return;
              ...
              <tr key={u.id}>
                ...
                <td
                  style={{
                    ...
                    textAlign: "center",
                    fontWeight: 600,
                    color: u.predictions > 0 ? "var(--gold)" : "var(--muted)",
                  }}
                >
                  {u.predictions}/72
                </td>
                <td
                  style={{
                    ...
                    textAlign: "center",
                    color:
                      u.bracket_picks > 0 ? "var(--cream)" : "var(--muted)",
                  }}
                >
                  {u.bracket_picks}
                </td>
                ...
                <td style={td}>
                  <div style={{ display: "flex", gap: 4 }}>
                    <button
                      ...
                      style={{
                        ...btnSmall,
                        background: u.is_unlocked
                          ? "rgba(212,168,67,0.15)"
                          : "var(--surface2)",
                        borderColor: u.is_unlocked
                          ? "var(--gold)"
                          : "var(--border)",
                        color: u.is_unlocked ? "var(--gold)" : "var(--dim)",
                      }}
                      title={
                        u.is_unlocked
                          ? "Lock predictions"
                    ...
                    <button
                      onClick={() => resetUser(u.id, u.display_name)}
                      style={btnSmall}
                      title="Reset predictions & scores"
                    >
                      🔄
                    </button>
                    <button
                    ...
                    >
                      ...
                      <td
                        style={{
                          ...
                          fontSize: 18,
                          color:
                            m.total_points > 0 ? "var(--gold)" : "var(--muted)",
                        }}
                      >
                        {m.total_points} pts
                      </td>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      >
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              setFilter((f) => ({ ...f, match_id: e.target.value }))
            }
          />
        </div>
        <div>
      ...
      {loading ? (
        ...
        >
          Use the filters above to search predictions. Try entering a match ID
          like &quot;a1&quot;.
        </div>
      )}
    </div>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{ ...card, padding: 24 }}>
        ...
        >
          <div>
            ...
            <p style={{ fontSize: 13, color: "var(--muted)", marginTop: 4 }}>
              Compare stored totals against the active scoring formula for every
              user.
            </p>
          </div>
          <button
            onClick={loadScoringComparison}
            disabled={loadingComparison}
            style={{ ...btnGold, opacity: loadingComparison ? 0.6 : 1 }}
          >
        ...
        {!comparison && !error && (
          ...
          >
            Click Load Comparison to calculate old vs new points from finished
            matches.
          </div>
        )}
        ...
        {comparison && (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            >
              ...
              >
                {comparison.comparison_version ?? "unknown"}
              </span>
              ...
              {[
                {
                  ...
                  value: comparison.finished_matches,
                },
                {
                  label: "Current stored",
                  value: comparison.totals.current_total,
                },
                { label: "New formula", value: comparison.totals.new_total },
                { label: "Old formula", value: comparison.totals.legacy_total },
                {
                  label: "Stored mismatches",
                  value: comparison.stored_new_mismatches ?? 0,
                },
              ].map((item) => (
                <div
                  key={item.label}
                  ...
                  {comparison.rows.map((row) => (
                    <tr key={row.user_id}>
                      <td style={td}>
                        ...
                        <div style={{ fontSize: 10, color: "var(--muted)" }}>
                          {row.played_matches ?? 0} scored matches
                          {row.selected_group_id
                            ? ` · group ${String(row.selected_group_id).slice(0, 8)}`
                            : ""}
                        </div>
                      </td>
                      <td
                      ...
                      >
                        {row.new_total}
                        <div style={{ fontSize: 10, color: "var(--muted)" }}>
                          match {row.new_match_points ?? row.new_total}
                        </div>
                      </td>
                      <td
                        style={{
                      ...
                      >
                        {row.legacy_total}
                        <div style={{ fontSize: 10, color: "var(--muted)" }}>
                          match {row.legacy_match_points ?? row.legacy_total}
                        </div>
                      </td>
                      <td
                        style={{
                          ...
                          fontFamily: "DM Mono, monospace",
                          color:
                            row.delta_current_vs_new === 0
                              ? "var(--green-bright)"
                              : "#e05c4a",
                        }}
                      >
                        {row.delta_current_vs_new > 0 ? "+" : ""}
                        {row.delta_current_vs_new}
                      </td>
                      <td
                        style={{
                          ...td,
                          ...
                          fontFamily: "DM Mono, monospace",
                          color:
                            row.delta_new_vs_legacy >= 0
                              ? "var(--green-bright)"
                              : "#e05c4a",
                        }}
                      >
                        {row.delta_new_vs_legacy > 0 ? "+" : ""}
                        {row.delta_new_vs_legacy}
                      </td>
                    </tr>
                  ))}
                </tbody>
...
function ToolsTab() {
  ...
  const reset = async (full: boolean) => {
    ...
    setResetting(true);
    setLog((prev) => [
      `🔄 Running ${full ? "full" : "partial"} reset...`,
      ...prev,
    ]);
  ...
  const loadScoringComparison = async () => {
    ...
    if (res.ok) {
      setComparison(data);
      setLog((prev) => [
        `Loaded comparison for ${data.users} users and ${data.finished_matches} finished matches`,
        ...prev,
      ]);
    } else {
      setLog((prev) => [
        `Error: ${data.error ?? "Could not load scoring comparison"}`,
        ...prev,
      ]);
    }
    setLoadingComparison(false);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          >
            ...
            <div>
              ...
              <p style={{ fontSize: 12, color: "var(--muted)", marginTop: 2 }}>
                Finishes Group A matches with fake results, generates random
                predictions for all users, and runs scoring.
              </p>
            </div>
          </div>
          <div
        ...
        <div style={{ ...card, padding: 24 }}>
          ...
          >
            ...
            <div>
              ...
              <p style={{ fontSize: 12, color: "var(--muted)", marginTop: 2 }}>
                Clean up test data. Choose partial (test matches only) or full
                (everything).
              </p>
            </div>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            >
              {resetting
                ? "⏳ Resetting..."
                : "🔄 Partial Reset (test matches only)"}
            </button>
            <button
              onClick={() => reset(true)}
              disabled={resetting}
      ...
      >
        ...
        >
          ...
          <div>
            ...
            <p style={{ fontSize: 12, color: "var(--muted)", marginTop: 2 }}>
              Forcefully overwrites the Supabase matches table with the latest
              statuses and scores from the external live APIs.
            </p>
          </div>
        </div>
        <button
          onClick={async () => {
            setLog((prev) => [
              "🔄 Syncing matches with external API...",
              ...prev,
            ]);
            const res = await fetch("/api/admin/sync-all-matches", {
              method: "POST",
            });
            const data = await res.json();
            setLog((prev) => [
              res.ok
                ? `✅ Done! ${data.updatedCount} matches updated.`
                : `❌ Error: ${data.error}`,
      ...
      <div style={{ ...card, padding: 24 }}>
        ...
        >
          ...
          <div>
            ...
            >
              Recalculate Bracket Bonuses
            </h3>
            <p style={{ fontSize: 12, color: "var(--muted)", marginTop: 2 }}>
              Scans all finished knockout matches and updates bracket bonus
              points for each player based on their original tournament
              prediction.
            </p>
          </div>
        </div>
        <button
          onClick={async () => {
            ...
            setLog((prev) => [
              res.ok
                ? `✅ Done! ${data.processedUsers} users, ${data.updatedScoreRows} score rows updated`
                : `❌ Error: ${data.error}`,
              ...prev,
            ]);
          }}
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <div>
              ...
              <p style={{ fontSize: 12, color: "var(--muted)", marginTop: 2 }}>
                Admin-only view comparing stored totals with the active formula.
              </p>
            </div>
          </div>
          <button
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            >
              {[
                ...
                {
                  label: "Legacy total",
                  value: comparison.totals.legacy_total,
                },
                {
                  label: "Delta",
                  value:
                    comparison.totals.new_total -
                    comparison.totals.legacy_total,
                },
              ].map((item) => (
                <div
                  key={item.label}
      ...
      <div style={{ ...card, padding: 16 }}>
        ...
        >
          ...
          <button onClick={() => setLog([])} style={btnSmall}>
            Clear
          </button>
        </div>
        <div
        ...
        >
          {log.length === 0 ? (
            <span style={{ color: "var(--muted)" }}>
              No actions yet. Run a simulation or reset to see output here.
            </span>
          ) : (
            log.map((line, i) => (
              <div
