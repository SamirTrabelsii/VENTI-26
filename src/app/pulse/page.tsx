...
import { GROUP_MATCHES, KNOCKOUT_MATCHES, getFlagUrl, TEAMS } from '@/lib/wc2026-data'
...
export default async function PulsePage() {
    ...
    const teamGoalsData: Record<string, { totalGoals: number, matchCount: number }> = {}
    const matchDict: Record<string, any> = {}
    ALL_MATCHES.forEach(m => { matchDict[m.id] = m })
    ...
    allPreds.forEach((p: any) => {
        ...
        if (m) {
            if (!teamGoalsData[m.home_team]) teamGoalsData[m.home_team] = { totalGoals: 0, matchCount: 0 }
            teamGoalsData[m.home_team].totalGoals += p.home_score
            teamGoalsData[m.home_team].matchCount += 1

            if (!teamGoalsData[m.away_team]) teamGoalsData[m.away_team] = { totalGoals: 0, matchCount: 0 }
            teamGoalsData[m.away_team].totalGoals += p.away_score
            teamGoalsData[m.away_team].matchCount += 1
        }
    })

    const avgPredictedGoals = totalPreds ? (totalPredictedGoals / totalPreds).toFixed(2) : '0.00'

    let totalRealGoals = 0
    let finishedCount = 0
    dbMatches.filter(m => m.status === 'finished').forEach(m => {
        if (typeof m.home_score === 'number' && typeof m.away_score === 'number') {
            totalRealGoals += m.home_score + m.away_score
            finishedCount++
        }
    })
    const avgRealGoals = finishedCount ? (totalRealGoals / finishedCount).toFixed(2) : '0.00'
    ...
    const teamActualGoals: Record<string, { totalGoals: number, matchCount: number }> = {}
    dbMatches.filter(m => m.status === 'finished' && typeof m.home_score === 'number' && typeof m.away_score === 'number').forEach(m => {
        // Find the static match data to get team codes
        const staticMatch = ALL_MATCHES.find(sm => sm.id === m.id)
        if (!staticMatch) return
        if (!teamActualGoals[staticMatch.home_team]) teamActualGoals[staticMatch.home_team] = { totalGoals: 0, matchCount: 0 }
        teamActualGoals[staticMatch.home_team].totalGoals += m.home_score
        teamActualGoals[staticMatch.home_team].matchCount += 1

        if (!teamActualGoals[staticMatch.away_team]) teamActualGoals[staticMatch.away_team] = { totalGoals: 0, matchCount: 0 }
    ...
    const lethalAttacks = Object.entries(teamGoalsData)
        .map(([teamCode, data]) => {
            ...
            return {
                ...
                actualAvg: actual && actual.matchCount > 0 ? (actual.totalGoals / actual.matchCount) : null
            }
        })
    ...
    const heatmap: { isCorrect: boolean; matchId: string; homeTeam: string; awayTeam: string; homeScore: number; awayScore: number }[] = []
    ...
    ALL_MATCHES.forEach(match => {
        ...
        const diff = Math.abs(hp - 33.3) + Math.abs(dp - 33.3) + Math.abs(ap - 33.3)
        ...
        if (isFinished && dbMatch.home_score !== null && dbMatch.away_score !== null) {
            ...
            const mostPredictedOutcome = hp > dp && hp > ap ? 'HOME_WIN' : ap > hp && ap > dp ? 'AWAY_WIN' : 'DRAW'
            heatmap.push({
        ...
        matchAnalysis.push({
            ...
            hp, dp, ap, conviction, convictionPick, polarizationScore,
            isFinished, actualOutcomePct,
            realHomeScore: dbMatch?.home_score, realAwayScore: dbMatch?.away_score,
            avgMatchGoals, zeroZeroPct
        })
    })

    const upcomingAnalysis = matchAnalysis.filter(m => !m.isFinished && m.totalPreds > 0)

    const highestConviction = [...upcomingAnalysis].sort((a, b) => b.conviction - a.conviction)[0]
    const mostPolarizing = [...upcomingAnalysis].sort((a, b) => b.polarizationScore - a.polarizationScore)[0]
    const goalFest = [...upcomingAnalysis].sort((a, b) => b.avgMatchGoals - a.avgMatchGoals)[0]
    const snoozeFest = [...upcomingAnalysis].sort((a, b) => b.zeroZeroPct - a.zeroZeroPct)[0]

    const finishedAnalysis = matchAnalysis.filter(m => m.isFinished && m.actualOutcomePct !== null)
    const biggestUpset = [...finishedAnalysis].sort((a, b) => a.actualOutcomePct - b.actualOutcomePct)[0]
    ...
    const golden4 = Object.entries(sfUserTeams)
        .map(([team_code, users]) => ({
            team_code,
            percentage: totalSfUsers > 0 ? Math.round((users.size / totalSfUsers) * 100) : 0,
            name: TEAMS.find(t => t.code === team_code)?.name || team_code
        }))
    ...
    return (
        <div style={{ minHeight: '100vh', background: 'var(--black)', color: 'var(--cream)' }}>
            ...
            <div className="resp-padding" style={{ maxWidth: 1200, margin: '0 auto', padding: '100px 5% 60px' }}>
                ...
                {/* ============================================= */}
                <div style={{ background: 'var(--surface2)', borderRadius: 20, padding: '32px 24px 24px', border: '1px solid var(--border)', marginBottom: 24 }}>
                    <h2 style={{ fontSize: 13, fontWeight: 700, color: 'var(--dim)', textTransform: 'uppercase', letterSpacing: 1.5, textAlign: 'center', marginBottom: 32 }}>
                        Consensus Champions
                    </h2>
                    ...
                    {topWinners.length >= 3 ? (
                            ...
                            <div className="pulse-podium" style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'center', gap: 16, marginBottom: 32 }}>
                                ...
                                <div className="pulse-podium-2" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: 140 }}>
                                    ...
                                    <div className="pulse-podium-block" style={{
                                        ...
                                        fontSize: 32, fontWeight: 800, fontFamily: 'Bebas Neue', color: 'rgba(0,0,0,0.5)'
                                    }}>2</div>
                                </div>
                                ...
                                <div className="pulse-podium-1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: 160 }}>
                                    ...
                                    <div className="pulse-podium-block" style={{
                                        ...
                                        borderRadius: '8px 8px 0 0',
                                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                                        fontSize: 40, fontWeight: 800, fontFamily: 'Bebas Neue', color: 'rgba(0,0,0,0.4)'
                                    }}>1</div>
                                </div>
                                ...
                                <div className="pulse-podium-3" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: 140 }}>
                                    ...
                                    <div className="pulse-podium-block" style={{
                                        ...
                                        borderRadius: '8px 8px 0 0',
                                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                                        fontSize: 28, fontWeight: 800, fontFamily: 'Bebas Neue', color: 'rgba(0,0,0,0.4)'
                                    }}>3</div>
                                </div>
                            </div>
                                ...
                                <div className="pulse-runnerups" style={{ display: 'flex', justifyContent: 'center', gap: 32, borderTop: '1px solid var(--border)', paddingTop: 20 }}>
                                    {runnerUps.map((r, i) => (
                                        <div key={r.team_code} className="pulse-runnerup-item" style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                            ...
                                            <span style={{ fontSize: 16, fontWeight: 800, fontFamily: 'Bebas Neue', color: 'var(--dim)' }}>{r.percentage}%</span>
                                        </div>
                                    ))}
                                </div>
                            )}
                        ...
                        <div style={{ color: 'var(--muted)', textAlign: 'center', padding: '40px 0' }}>Not enough bracket predictions yet.</div>
                    )}
                </div>
                ...
                {/* ============================================= */}
                {golden4.length > 0 && (
                    <div style={{ background: 'var(--surface2)', borderRadius: 20, padding: '24px', border: '1px solid var(--border)', marginBottom: 24 }}>
                        <h2 style={{ fontSize: 13, fontWeight: 700, color: 'var(--dim)', textTransform: 'uppercase', letterSpacing: 1.5, textAlign: 'center', marginBottom: 8 }}>
                            Predicted Semifinalists
                        </h2>
                        <p style={{ fontSize: 12, color: 'var(--muted)', textAlign: 'center', marginBottom: 24 }}>
                        ...
                        <div className="pulse-golden4" style={{ display: 'flex', justifyContent: 'center', gap: 12, flexWrap: 'wrap' }}>
                            {golden4.map((team, i) => (
                                <div key={team.team_code} className="pulse-golden4-item" style={{
                                    ...
                                    border: i < 4 ? '1px solid rgba(212,168,67,0.2)' : '1px solid var(--border)',
                                    display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8,
                                    textAlign: 'center'
                                }}>
                                    <img src={getFlagUrl(team.team_code)} alt={team.team_code} width={40} height={30}
                                        style={{ borderRadius: 4, border: '1px solid var(--border)' }} />
                                    <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--cream)', lineHeight: 1.2 }}>{team.name}</div>
                                    <div style={{ fontSize: 22, fontWeight: 800, fontFamily: 'Bebas Neue', color: i < 4 ? 'var(--gold)' : 'var(--cream)', letterSpacing: 1 }}>{team.percentage}%</div>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                <div className="resp-bento-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 24 }}>
                    ...
                    <div style={{ background: 'var(--surface2)', borderRadius: 20, padding: 24, border: '1px solid var(--border)', display: 'flex', flexDirection: 'column' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
                            <h2 style={{ fontSize: 13, fontWeight: 700, color: 'var(--dim)', textTransform: 'uppercase', letterSpacing: 1.5 }}>Goal Expectancy</h2>
                            <div style={{ textAlign: 'right' }}>
                                <div style={{ fontSize: 24, fontWeight: 700, color: 'var(--gold)' }}>{avgPredictedGoals}</div>
                                <div style={{ fontSize: 11, color: 'var(--muted)', textTransform: 'uppercase' }}>Avg Predicted Goals</div>
                                <div style={{ fontSize: 11, color: 'var(--dim)', marginTop: 4 }}>vs {avgRealGoals} Actual</div>
                            </div>
                        </div>

                        {/* Bars rendered with pixel heights */}
                        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 12, borderBottom: '1px solid var(--border)', paddingBottom: 8 }}>
                            {bucketEntries.map(([label, count]) => {
                                const barHeight = Math.max(Math.round((count / maxBucketCount) * MAX_BAR_PX), 4)
                                const pct = totalPreds ? Math.round((count / totalPreds) * 100) : 0
                                const isHighlight = label === '2' || label === '3'
                                return (
                                    <div key={label} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                                        <div style={{ fontSize: 11, fontWeight: 700, color: pct > 0 ? 'var(--cream)' : 'var(--muted)' }}>{pct}%</div>
                                        <div style={{
                                            width: '100%', maxWidth: 36,
                                            height: barHeight,
                                            background: isHighlight ? 'var(--gold)' : 'var(--surface3)',
                        ...
                        <div style={{ display: 'flex', gap: 12, marginTop: 10 }}>
                            {bucketEntries.map(([label]) => (
                                <div key={label} style={{ flex: 1, fontSize: 12, fontWeight: 600, color: 'var(--dim)', textAlign: 'center' }}>{label}</div>
                            ))}
                        </div>
                        <div style={{ textAlign: 'center', fontSize: 10, color: 'var(--muted)', textTransform: 'uppercase', marginTop: 8 }}>Total Goals in Match</div>
                    </div>
                    ...
                    <div className="pulse-bento-card" style={{ background: 'var(--surface2)', borderRadius: 20, padding: 24, border: '1px solid var(--border)' }}>
                        ...
                        <div style={{ display: 'flex', gap: 16, marginBottom: 20 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: 'var(--muted)' }}>
                                <div style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--gold)' }}></div> Predicted
                            </div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: 'var(--muted)' }}>
                                <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#10b981' }}></div> Actual
                            </div>
                        </div>
                                ...
                                <div key={team.teamCode} className="pulse-lethal-row" style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                                    ...
                                    <div className="pulse-lethal-stats" style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                                        <div style={{ textAlign: 'center' }}>
                                            ...
                                            <div style={{ fontSize: 9, color: 'var(--gold)', textTransform: 'uppercase', fontWeight: 600 }}>Pred</div>
                                        </div>
                                        <div style={{ width: 1, height: 28, background: 'var(--border)' }}></div>
                                        <div style={{ textAlign: 'center' }}>
                                            <div style={{ fontSize: 18, fontWeight: 800, color: team.actualAvg !== null ? '#10b981' : 'var(--dim)', fontFamily: 'Bebas Neue', letterSpacing: 1 }}>
                                                {team.actualAvg !== null ? team.actualAvg.toFixed(2) : '—'}
                                            </div>
                                            <div style={{ fontSize: 9, color: '#10b981', textTransform: 'uppercase', fontWeight: 600 }}>Actual</div>
                                        </div>
                                    </div>
                                </div>
                            )) : (
                                <div style={{ color: 'var(--muted)', textAlign: 'center', padding: '20px 0' }}>Not enough predictions yet.</div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="pulse-bento-card" style={{ background: 'var(--surface2)', borderRadius: 20, padding: 24, border: '1px solid var(--border)' }}>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, alignContent: 'flex-start' }}>
                            {heatmap.length > 0 ? heatmap.map((h, i) => (
                                ...
                                <div style={{ color: 'var(--muted)', fontSize: 13 }}>No matches finished yet.</div>
                            )}
                        </div>
                        ...
                        {heatmap.length > 0 && (
                            <div style={{ marginTop: 24, display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1px solid var(--border)', paddingTop: 16 }}>
                                <div style={{ fontSize: 12, color: 'var(--dim)' }}>Crowd Accuracy</div>
                                <div style={{ fontSize: 24, fontWeight: 800, color: 'var(--cream)', fontFamily: 'Bebas Neue', letterSpacing: 1 }}>
                                    {Math.round((heatmap.filter(h => h.isCorrect).length / heatmap.length) * 100)}%
                                </div>
                            </div>
                        )}
                    </div>
                        ...
                        <div className="pulse-bento-card" style={{ background: 'var(--surface2)', borderRadius: 20, padding: 24, border: '1px solid var(--border)' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                    <img src={getFlagUrl(highestConviction.match.home_team)} width={28} height={20} alt="home" />
                                    <span style={{ fontWeight: 600 }}>{highestConviction.match.home_team}</span>
                                </div>
                                <span style={{ fontSize: 12, color: 'var(--dim)' }}>vs</span>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                    <span style={{ fontWeight: 600 }}>{highestConviction.match.away_team}</span>
                                    <img src={getFlagUrl(highestConviction.match.away_team)} width={28} height={20} alt="away" />
                                </div>
                            </div>

                            <div style={{ background: 'var(--surface3)', borderRadius: 12, padding: 16, textAlign: 'center', border: '1px solid rgba(255,255,255,0.05)' }}>
                                <div style={{ fontSize: 40, fontFamily: 'Bebas Neue', color: 'var(--gold)', lineHeight: 1 }}>{Math.round(highestConviction.conviction)}%</div>
                                <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--cream)', marginTop: 8 }}>Pick {highestConviction.convictionPick} to Win</div>
                            </div>
                        </div>
                    )}
                    ...
                    {snoozeFest && (
                        <div className="pulse-bento-card" style={{ background: 'linear-gradient(145deg, #1c2e4a 0%, var(--surface2) 100%)', borderRadius: 20, padding: 24, border: '1px solid rgba(66, 165, 245, 0.3)' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                    <img src={getFlagUrl(snoozeFest.match.home_team)} width={28} height={20} alt="home" />
                                    <span style={{ fontWeight: 600 }}>{snoozeFest.match.home_team}</span>
                                </div>
                                <span style={{ fontSize: 12, color: 'var(--dim)' }}>vs</span>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                    <span style={{ fontWeight: 600 }}>{snoozeFest.match.away_team}</span>
                                    <img src={getFlagUrl(snoozeFest.match.away_team)} width={28} height={20} alt="away" />
                                </div>
                            </div>
                            <div style={{ fontSize: 32, fontFamily: 'Bebas Neue', color: 'var(--cream)', lineHeight: 1 }}>
                                {Math.round(snoozeFest.zeroZeroPct)}% <span style={{ fontSize: 16, color: 'var(--dim)', fontFamily: 'Inter' }}>Predict 0-0</span>
                            </div>
                        </div>
                    )}
                    ...
                    {goalFest && (
                        <div className="pulse-bento-card" style={{ background: 'linear-gradient(145deg, #4a1c1c 0%, var(--surface2) 100%)', borderRadius: 20, padding: 24, border: '1px solid rgba(229, 57, 53, 0.3)' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                    <img src={getFlagUrl(goalFest.match.home_team)} width={28} height={20} alt="home" />
                                    <span style={{ fontWeight: 600 }}>{goalFest.match.home_team}</span>
                                </div>
                                <span style={{ fontSize: 12, color: 'var(--dim)' }}>vs</span>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                    <span style={{ fontWeight: 600 }}>{goalFest.match.away_team}</span>
                                    <img src={getFlagUrl(goalFest.match.away_team)} width={28} height={20} alt="away" />
                                </div>
                            </div>
                            <div style={{ fontSize: 32, fontFamily: 'Bebas Neue', color: 'var(--cream)', lineHeight: 1 }}>
                                {goalFest.avgMatchGoals.toFixed(2)} <span style={{ fontSize: 16, color: 'var(--dim)', fontFamily: 'Inter' }}>Goals/Game</span>
                            </div>
                        </div>
                    )}
                    ...
                    {mostPolarizing && (
                        <div className="pulse-bento-card" style={{ background: 'var(--surface2)', borderRadius: 20, padding: 24, border: '1px solid var(--border)' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                    <img src={getFlagUrl(mostPolarizing.match.home_team)} width={28} height={20} alt="home" />
                                    <span style={{ fontWeight: 600 }}>{mostPolarizing.match.home_team}</span>
                                </div>
                                <span style={{ fontSize: 12, color: 'var(--dim)' }}>vs</span>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                    <span style={{ fontWeight: 600 }}>{mostPolarizing.match.away_team}</span>
                                    <img src={getFlagUrl(mostPolarizing.match.away_team)} width={28} height={20} alt="away" />
                                </div>
                            </div>

                            {/* Split Bar */}
                            <div style={{ display: 'flex', height: 16, borderRadius: 8, overflow: 'hidden', marginBottom: 12 }}>
                                <div style={{ width: `${mostPolarizing.hp}%`, background: 'var(--gold)' }}></div>
                                <div style={{ width: `${mostPolarizing.dp}%`, background: 'var(--dim)' }}></div>
                        ...
                        <div className="pulse-bento-card pulse-bento-span-2" style={{ background: 'var(--surface2)', borderRadius: 20, padding: 24, border: '1px solid var(--border)' }}>
                            ...
                            <div className="resp-upset-row" style={{ display: 'flex', alignItems: 'center', gap: 24, background: 'var(--black)', padding: '16px 24px', borderRadius: 16, border: '1px solid var(--border)' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                                    ...
                                    <img src={getFlagUrl(biggestUpset.match.away_team)} width={32} height={24} alt="away" style={{ borderRadius: 4 }} />
                                </div>

                                <div style={{ width: 1, height: 40, background: 'var(--border)' }}></div>

                                <div>
                                    <div style={{ fontSize: 11, color: 'var(--dim)', textTransform: 'uppercase', fontWeight: 600, letterSpacing: 1 }}>Predicted By</div>
                                    <div style={{ fontSize: 24, fontWeight: 800, color: '#e53935' }}>Only {Math.round(biggestUpset.actualOutcomePct)}%</div>
                                </div>
                            </div>
                        </div>
                    )}
