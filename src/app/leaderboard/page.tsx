...
function buildScopedLeaderboard(
  ...
  groupPredData: any[],
  bracketPredData: any[],
  scope: Omit<LeaderboardScope, "users" | "finished_count" | "match_count"> & {
    filter: (match: any) => boolean;
    matchTotal: number;
  },
): LeaderboardScope {
  ...
  try {
    ...
    for (const apiMatch of data.matches ?? []) {
      ...
      finished.push({
        ...
        qualifier:
          dbMatch?.qualifier ?? apiQualifier ?? staticMatch.qualifier ?? null,
...
export default async function LeaderboardPage() {
  ...
  for (const row of groupPredData) {
    groupPredCounts.set(
      row.user_id,
      (groupPredCounts.get(row.user_id) ?? 0) + 1,
    );
  }
  ...
  for (const row of bracketPredData) {
    bracketPredCounts.set(
      row.user_id,
      (bracketPredCounts.get(row.user_id) ?? 0) + 1,
    );
  }
  ...
  const scopeDefinitions = [
    ...
    },
    {
      ...
      filter: (match: any) =>
        ["R32", "R16", "QF", "SF", "3RD", "FINAL"].includes(match.group_label),
    },
    {
      key: "gw1",
      type: "round" as const,
    ...
    },
    {
      ...
      matchTotal: 2,
      filter: (match: any) =>
        match.group_label === "FINAL" || match.group_label === "3RD",
    },
  ...
  if (liveMatches.length > 0) {
    ...
    const liveGroupPredictions = await fetchAllRows(
      supabase
        .from("predictions")
        .select("*")
        .in("match_id", Array.from(liveMatchIds)),
    );
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--black)" }}>
      ...
      <div
        style={{ maxWidth: 1000, margin: "0 auto", padding: "100px 5% 60px" }}
      >
        <div style={{ marginBottom: 40, textAlign: "center" }}>
          ...
          <p style={{ color: "var(--muted)", marginTop: 8 }}>
            {leaderboard.length} registered predictor
            {leaderboard.length !== 1 ? "s" : ""} · {TOTAL_MATCHES} matches to
            predict
          </p>
        </div>
