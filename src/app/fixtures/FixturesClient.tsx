...
function inferQualifier(match: any): string | null {
  ...
  if (
    ...
    if (match.actualHomeScore > match.actualAwayScore)
      return match.home_team ?? null;
    if (match.actualAwayScore > match.actualHomeScore)
      return match.away_team ?? null;
  }
  return null;
}
...
function formatPlaceholder(code: string): string {
  ...
  if (/^\d[A-L]$/.test(code)) {
    const pos =
      code[0] === "1" ? "1st" : code[0] === "2" ? "2nd" : `${code[0]}th`;
...
function getKnockoutPickSlot(
  matchId: string,
): { round: string; slotIndex: number } | null {
...
export default function FixturesClient({
  ...
  useEffect(() => {
    for (const lm of liveApiMatches) {
      const dbMatch = currentDbMatches.find(
        (m) =>
          m.home_team === lm.homeTeam?.tla && m.away_team === lm.awayTeam?.tla,
      );
      if (!dbMatch) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel("fixtures_matches")
      .on(
        "postgres_changes",
        { event: "UPDATE", schema: "public", table: "matches" },
        (payload) => {
          setCurrentDbMatches((prev) =>
            prev.map((m) =>
  ...
  const fullMatches = useMemo(() => {
    return ALL_MATCHES.map((m) => {
      ...
      const apiMatch = dbMatch
        ? liveApiMatches.find(
            (l) =>
              l.homeTeam?.tla === dbMatch.home_team &&
              l.awayTeam?.tla === dbMatch.away_team,
          )
        : null;
      ...
      if (apiMatch) {
        ...
        if (apiMatch.score?.went_to_penalties || apiMatch.score?.penalties) {
          ...
          penaltyHomeScore =
            apiMatch.score?.penalties?.home ?? penaltyHomeScore;
          penaltyAwayScore =
            apiMatch.score?.penalties?.away ?? penaltyAwayScore;
        }
  ...
  const filteredMatches = useMemo(() => {
    if (filter === "group")
      return fullMatches.filter((m) => !m.isKnockoutMatch);
    if (filter === "knockout")
      return fullMatches.filter((m) => m.isKnockoutMatch);
    return fullMatches;
  }, [fullMatches, filter]);
  ...
  const sortedDateGroups = useMemo(() => {
    ...
    const sorted = [...filteredMatches].sort(
      (a, b) =>
        getAdjustedKickoff(a.kickoff).getTime() -
        getAdjustedKickoff(b.kickoff).getTime(),
    );
    // Group into an array of { date, isoDate, matches } in chronological order
    const result: {
      date: string;
      isoDate: string;
      matches: typeof fullMatches;
    }[] = [];
  ...
  const selectedModalMatch = useMemo<ModalMatchData | null>(() => {
    ...
    return {
      ...
      homeTeam: {
        name:
          homeTeam?.name ?? (ko ? formatPlaceholder(m.home_team) : m.home_team),
        shortName: homeTeam?.code ?? m.home_team,
        tla: m.home_team,
        crest: homeTeam
          ? `https://flagcdn.com/w80/${homeTeam.iso2}.png`
          : undefined,
      },
      awayTeam: {
        name:
          awayTeam?.name ?? (ko ? formatPlaceholder(m.away_team) : m.away_team),
        shortName: awayTeam?.code ?? m.away_team,
        tla: m.away_team,
        crest: awayTeam
          ? `https://flagcdn.com/w80/${awayTeam.iso2}.png`
          : undefined,
      },
      score: {
  ...
  const MatchCard = ({ m }: { m: any }) => {
    ...
    if (ko) {
      ...
      if (slot) {
        const koPick = liveKoPicks.find(
          (p) => p.round === slot.round && p.slot_index === slot.slotIndex,
        );
        if (koPick) {
          prediction = koPick;
        }
      ...
      <div
        ...
        style={{
          background: isLive
            ? "rgba(200,57,43,0.03)"
            : ko
              ? "linear-gradient(135deg, var(--surface) 0%, rgba(212,168,67,0.03) 100%)"
              : "var(--surface)",
      ...
      >
        ...
        >
          ...
          {isLive ? (
            ...
            >
              {getAdjustedKickoff(m.kickoff).toLocaleTimeString("en-US", {
                hour: "2-digit",
                minute: "2-digit",
                hour12: false,
                timeZone: "Africa/Tunis",
              })}{" "}
              <span style={{ fontSize: 9, opacity: 0.7 }}>GMT+1</span>
            </span>
          )}
        </div>
        ...
        >
          ...
          >
            {homeTeam ? (
              ...
              <span style={{ fontSize: 28, width: 42, textAlign: "center" }}>
                🏳️
              </span>
            )}
            <span
            ...
            >
              <span
                style={{
                  ...
                  color: isLive || isFinished ? "var(--gold)" : "var(--muted)",
                  width: 24,
                  textAlign: "center",
                }}
              >
              ...
              <span
                style={{ fontSize: 14, color: "var(--muted)", opacity: 0.5 }}
              >
                :
              </span>
              <span
    ...
    <div style={{ maxWidth: 1000, margin: "0 auto", padding: "0 40px 60px" }}>
      ...
      <div style={{ marginBottom: 40 }}>
        ...
        <p style={{ color: "var(--muted)", fontSize: 16, marginTop: 8 }}>
          {groupCount} group stage + {koCount} knockout = {groupCount + koCount}{" "}
          total matches
        </p>
      </div>
          ...
          {(
            ...
            <button
              ...
              style={{
                ...
                fontWeight: 600,
                background:
                  filter === tab.key ? "var(--surface3)" : "transparent",
        ...
        <div style={{ marginBottom: 48 }}>
          <div
            style={{
              ...
              gap: 12,
              marginBottom: 20,
              borderBottom: "1px solid rgba(200,57,43,0.3)",
              paddingBottom: 12,
            }}
          >
            <div
      ...
      {selectedModalMatch && (
        <MatchModal
          ...
          prediction={(() => {
            ...
            if (ko) {
              const slot = getKnockoutPickSlot(
                selectedModalMatch.id.toString(),
              );
              if (slot) {
                return (
                  liveKoPicks.find(
                    (p) =>
                      p.round === slot.round && p.slot_index === slot.slotIndex,
                  ) ?? null
                );
              }
            }
            return (
              predictions.find((p) => p.match_id === selectedModalMatch.id) ??
              null
            );
          })()}
      ...
      <button
        ...
        style={{
          ...
          boxShadow:
            "0 10px 40px rgba(0,0,0,0.5), 0 0 20px rgba(212,168,67,0.2)",
