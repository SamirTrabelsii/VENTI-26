...
> = {
  bronze: {
    ...
    border: "rgba(205,127,50,0.32)",
    bg: "rgba(205,127,50,0.06)",
  },
  silver: {
    label: "Silver",
    color: "#b0b8c8",
    glow: "rgba(176,184,200,0.22)",
    border: "rgba(176,184,200,0.32)",
    bg: "rgba(176,184,200,0.06)",
  },
  gold: {
    label: "Gold",
    color: "#d4a843",
    glow: "rgba(212,168,67,0.28)",
    border: "rgba(212,168,67,0.42)",
    bg: "rgba(212,168,67,0.09)",
  },
  diamond: {
    label: "Diamond",
    color: "#5b9fff",
    glow: "rgba(91,159,255,0.28)",
    border: "rgba(91,159,255,0.42)",
    bg: "rgba(91,159,255,0.09)",
  },
  crown: {
    label: "Crown",
    color: "#e05c4a",
    glow: "rgba(224,92,74,0.28)",
    border: "rgba(224,92,74,0.42)",
    bg: "rgba(224,92,74,0.09)",
  },
  lightning: {
    label: "Lightning",
    color: "#a855f7",
...
const DNA_PROFILES = [
  {
    ...
    color: "#d4a843",
    description:
      "Exact scores, precise margins. You do not just predict results — you predict scorelines. Patience and precision define you.",
  },
  {
    id: "romantic",
    icon: "🎨",
    ...
    color: "#e05c4a",
    description:
      "You believe in beautiful, attacking football. High-scoring games, bold picks, big names winning big matches.",
  },
  {
    id: "contrarian",
    icon: "🐺",
    ...
    color: "#22c55e",
    description:
      "Underdogs, upsets, dark horses. You love going against the grain — and when you are right, the points are magnificent.",
  },
  {
    id: "pragmatist",
    icon: "🧱",
    ...
    color: "#5b9fff",
    description:
      "Draws, tight margins, defensive outcomes. You respect the grind. You know that 1-0 is the most honest scoreline in football.",
  },
];
...
function ProfileContent() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const { data: liveKoPicks } = await supabase
        .from("live_ko_picks")
        .select("*")
        .eq("user_id", targetUserId);
      ...
      for (const p of normalizedLiveKoPicks)
      ...
      const dbMatches = (allDbMatches || []).filter(
      ...
      for (const lm of activeLiveMatches) {
        const dbMatch = (allDbMatches || []).find(
          (m: any) =>
            m.home_team === lm.homeTeam?.tla &&
            m.away_team === lm.awayTeam?.tla,
        );
        if (!dbMatch) {
        ...
        const realQualifier =
          ...
          inferQualifier({
            ...
            penalty_home_score:
              lm.score?.penalties?.home ?? dbMatch.penalty_home_score,
            penalty_away_score:
              lm.score?.penalties?.away ?? dbMatch.penalty_away_score,
            went_to_penalties:
              lm.score?.went_to_penalties || dbMatch.went_to_penalties,
          }) ??
          staticMatch.qualifier ??
          null;
        const wentToPenalties = Boolean(
          lm.score?.went_to_penalties ||
          lm.score?.penalties ||
          dbMatch.went_to_penalties,
        );
        const penaltyHomeScore =
          lm.score?.penalties?.home ?? dbMatch.penalty_home_score ?? null;
        const penaltyAwayScore =
          lm.score?.penalties?.away ?? dbMatch.penalty_away_score ?? null;
      ...
      for (const dbm of dbMatches) {
        ...
        if (pred && typeof pred.home_score === "number") {
          ...
          const res = scoreMatch(
            ...
            {
              predQualifier: pred.qualifier_pick ?? pred.team_code ?? null,
              realQualifier: isKnockout
                ? (inferQualifier(dbm) ?? staticMatch?.qualifier ?? null)
                : null,
            },
          );
          hist.push({
      ...
      for (const m of hist) {
        ...
        if (m.status === "FINISHED" || m.status === "finished") {
          if (m.type === "exact" || m.type === "correct") {
            dynCurrentStreak++;
            if (dynCurrentStreak > dynBestStreak)
              dynBestStreak = dynCurrentStreak;
          } else {
            dynCurrentStreak = 0;
          }
        }
  ...
  const finishedPredictedMatches = historyMatches.filter(
    (m) =>
  ...
  const resultAccuracy =
    finishedPredictedMatches.length > 0
      ? Math.round(
          (finishedCorrectOutcomeMatches.length /
            finishedPredictedMatches.length) *
            100,
        )
      : 0;
  ...
  if (loading) {
    return (
      ...
      >
        <Nav initials="PL" />
        <div style={{ fontSize: 14, color: "var(--muted)", paddingTop: 80 }}>
  ...
  const svgWidth = 800;
  const svgHeight = 200;
  const polylinePoints = pointsProgression
    .map((p, i) => {
    ...
    >
      ...
      <div style={{ position: "relative", overflow: "hidden", paddingTop: 64 }}>
        <div
          style={{
            ...
            inset: 0,
            pointerEvents: "none",
            background: `radial-gradient(ellipse 55% 40% at 15% 0%, rgba(212,168,67,0.09) 0%, transparent 60%), radial-gradient(ellipse 55% 40% at 85% 0%, rgba(212,168,67,0.09) 0%, transparent 60%)`,
          }}
        />
        <div
          style={{
        ...
        <div
          className="resp-flex-stack resp-padding"
          style={{
            maxWidth: 1200,
            margin: "0 auto",
        ...
        >
          <img
            ...
            style={{
              ...
              height: 120,
              borderRadius: "50%",
              background: profile?.avatar_color || "var(--surface2)",
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
              <span
                style={{ fontSize: 16, color: "var(--gold)", fontWeight: 600 }}
              >
                {totalPoints} Points
              </span>
      ...
      >
        ...
        <Section label="Match History">
          ...
          >
            <div
              className="match-card-header resp-match-row-header"
              style={{
                display: "grid",
                    ...
                    >
                      ...
                      <div
                        ...
                        style={{
                          ...
                          fontFamily: "Bebas Neue",
                          fontSize: 24,
                          color: m.points > 0 ? "var(--gold)" : "var(--muted)",
                        }}
                      >
                        +{m.points}
                      </div>
        ...
        >
          <div style={{ display: "flex", flexDirection: "column", gap: 40 }}>
            {(Object.keys(TIER_CONFIG) as Tier[]).map((tierKey) => {
              const tierBadges = filteredBadges.filter(
                (b) => b.tier === tierKey,
              );
        ...
        <Section label="Prediction DNA">
          <div
            style={{
              background: `linear-gradient(140deg, ${dnaProfile.color}13 0%, var(--surface2) 55%)`,
    ...
    <div
      ...
      style={{
        ...
        boxShadow: badge.unlocked
          ? `0 0 24px ${t.glow}, inset 0 0 12px ${t.glow}`
          : "none",
      ...
      <div style={{ display: "flex", gap: 12 }}>
        <div
          style={{
            ...
            height: 48,
            borderRadius: 12,
            background: badge.unlocked ? `${t.color}1a` : "var(--surface3)",
