...
const TREE_WIDTH =
  TREE_COLUMN_COUNT * COLUMN_WIDTH + (TREE_COLUMN_COUNT - 1) * COLUMN_GAP;
...
function orderedFixtures(
  fixturesById: Map<string, LiveBracketFixture>,
  ids: readonly string[],
) {
  return ids
...
function getFixtureState(fixture: LiveBracketFixture) {
  const known =
    isKnownTeamCode(fixture.homeCode) && isKnownTeamCode(fixture.awayCode);
...
function draftFromPick(pick?: PickRow): DraftPick {
  return pick &&
    typeof pick.home_score === "number" &&
    typeof pick.away_score === "number"
    ? {
        home: pick.home_score ?? "",
        away: pick.away_score ?? "",
        advancing: pick.team_code || null,
      }
    : emptyDraft();
}
...
function draftReadyToSave(fixture: LiveBracketFixture, draft: DraftPick) {
  ...
  const tied =
    typeof draft.home === "number" &&
    typeof draft.away === "number" &&
  ...
  return (
    <input
      ...
      style={{
        ...
        borderRadius: 8,
        border:
          value !== "" ? "1px solid var(--gold)" : "1px solid var(--border)",
...
function FixtureCard({
  ...
  return (
    <div
      className={`fixture-tile ${isFinal ? "fixture-tile-final" : ""} ${isThirdPlace ? "fixture-tile-third" : ""}`}
      style={{
        border: isFinal
          ? "1px solid rgba(240,201,106,0.82)"
          : isThirdPlace
            ? "1px solid rgba(244,241,235,0.15)"
      ...
      <div
        style={{
          ...
          borderBottom: isFinal
            ? "1px solid rgba(240,201,106,0.28)"
            : "1px solid var(--border)",
        ...
        <div>
          <div
            style={{
              ...
              gap: 6,
              fontSize: 11,
              color: isFinal ? "var(--gold-light)" : "var(--gold)",
              fontWeight: 800,
              letterSpacing: 1.2,
              textTransform: "uppercase",
            }}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <TeamCell
              ...
              seed={fixture.homeSeed}
              selected={homeSelected}
            />
          </div>
          <ScoreInput
            value={draft.home}
            disabled={!known || locked}
            onChange={setHome}
          />
        </div>
...
function CompactBracketColumn({
  ...
  return (
    <div
      className={`bracket-column ${tone === "final" ? "bracket-column-final" : ""}`}
    >
      <div className="bracket-column-title">{title}</div>
      {fixtures.map((fixture, index) => {
        const key = pickKey(fixture);
        const top =
          COLUMN_HEADER_HEIGHT +
          positions[index] * SLOT_GAP +
          (SLOT_GAP - TILE_HEIGHT) / 2;
        return (
...
function ConnectorPath({
  ...
  const fromX = fromRight
    ? columnLeft(fromColumn) + COLUMN_WIDTH
    : columnLeft(fromColumn);
  const toX = fromRight
    ? columnLeft(toColumn)
    : columnLeft(toColumn) + COLUMN_WIDTH;
...
function BracketConnectors() {
  const pairPositions: [number, number][] = [
    [0, 1],
    [2, 3],
    [4, 5],
    [6, 7],
      ...
      <g>
        {pairPositions.map((pair, index) => (
          <ConnectorPath
            ...
            toColumn={1}
            fromPositions={pair}
            toPosition={R16_POSITIONS[index]}
          />
        ))}
        ...
        <SingleConnectorPath
          fromColumn={3}
          toColumn={4}
          fromPosition={SF_POSITIONS[0]}
          toPosition={FINAL_POSITIONS[0]}
        />
        <SingleConnectorPath
          fromColumn={5}
          toColumn={4}
...
export default function LiveBracketClient({
  ...
  const [picksByKey, setPicksByKey] = useState(() => {
    ...
    for (const pick of initialPicks)
      map[`${pick.round}_${pick.slot_index}`] = pick;
    return map;
  });
  const [drafts, setDrafts] = useState<Record<string, DraftPick>>(() => {
    ...
    for (const fixture of initialFixtures) {
      const saved = initialPicks.find(
        (pick) =>
          pick.round === fixture.round && pick.slot_index === fixture.slotIndex,
      );
  ...
  const waitingCount = fixtures.filter(
    (fixture) => !getFixtureState(fixture).known,
  ).length;
  const savedCount = Object.values(picksByKey).filter(
    (pick) =>
      typeof pick.home_score === "number" &&
      typeof pick.away_score === "number",
  ).length;
  ...
  const canSaveChanges =
    ...
    changedFixtures.every((fixture) =>
      draftReadyToSave(fixture, drafts[pickKey(fixture)] || emptyDraft()),
    );
  ...
  const saveAllChanges = async () => {
    ...
    try {
      ...
      setErrors((prev) => ({
        ...prev,
        general: error.message || "Could not save predictions",
      }));
    } finally {
      setSaving(false);
    }
  ...
  return (
    <main
      className="live-main live-bracket-page"
      style={{ maxWidth: 1400, margin: "0 auto", padding: "104px 20px 90px" }}
    >
      <div
        style={{
          display: "flex",
      ...
      >
        Hi {displayName}. Pick each knockout match once both teams are known.
        Your old bracket stays safe.
        {!currentUserId && " Sign in to save your picks."}
      </div>

      <section
        className="bracket-stage-shell"
        style={{
          border: "1px solid var(--border)",
      ...
      >
        ...
        >
          {currentUserId
            ? `${changedFixtures.length} unsaved pick${changedFixtures.length === 1 ? "" : "s"}`
      ...
      <style jsx global>{`
        ...
        .bracket-trophy-stage img {
          ...
          filter: drop-shadow(0 14px 22px rgba(0, 0, 0, 0.42))
            drop-shadow(0 0 14px rgba(212, 168, 67, 0.24));
        }
        ...
        .fixture-tile-final:hover {
          ...
          box-shadow:
            ...
            0 0 0 1px rgba(240, 201, 106, 0.38),
            0 0 42px rgba(212, 168, 67, 0.22) !important;
        }
