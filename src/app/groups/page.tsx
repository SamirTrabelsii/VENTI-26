...
export default function GroupsPage() {
    ...
    async (uid: string) => {
      ...
      const enriched: GroupWithMeta[] = await Promise.all(
        rawGroups.map(async (g) => {
          const res = await fetch(`/api/groups/${g.id}/scores`, {
            cache: "no-store",
          });
          const fresh = res.ok
            ? await res.json()
            : { scores: [], member_count: 0, members_preview: [] };
          ...
          return {
            ...
            leader_name:
              leader?.display_name ?? leader?.profile?.display_name ?? null,
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--black)" }}>
      ...
      <div
        style={{ maxWidth: 1200, margin: "0 auto", padding: "88px 40px 60px" }}
      >
        {/* Header */}
        <div
          style={{
        ...
        >
          <div>
            ...
            >
              Compete in private groups. Share your invite code and see who
              predicts best.
            </p>
          </div>
        ...
        {groups.length === 0 ? (
          ...
          >
            ...
            >
              Create your first group and invite friends to compete on
              predictions.
            </p>
          ...
          >
            ...
            >
              ...
              <span
                style={{ fontSize: 14, fontWeight: 500, color: "var(--dim)" }}
              >
                New group
              </span>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button
                style={{
                  ...S.btnPrimary,
                  opacity: loading || inviteCode.length < 6 ? 0.6 : 1,
                }}
...
// ── Group card ─────────────────────────────────────────────────────────────────
function GroupCard({ g }: { g: GroupWithMeta }) {
        ...
        >
          ...
          <div
            style={{
              ...
              borderRadius: 20,
              background:
                g.my_rank === 1 ? "rgba(212,168,67,0.15)" : "var(--surface2)",
        ...
        <div style={{ fontSize: 12 }}>
          ...
          {g.leader_name && g.my_rank !== 1 && (
            <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 2 }}>
              Leader:{" "}
              <span style={{ color: "var(--cream)" }}>{g.leader_name}</span> ·{" "}
              {g.leader_points} pts
            </div>
          )}
        </div>
