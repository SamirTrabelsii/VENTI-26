...
export default function GroupDetailPage() {
  ...
  const load = useCallback(
    async (_uid: string) => {
      ...
      const res = await fetch(`/api/groups/${id}/scores`, {
        cache: "no-store",
      });
      if (!res.ok) {
        router.push("/groups");
  ...
  const leaveGroup = async () => {
    ...
    const confirmed = window.confirm(
      `Leave "${group.name}"? You can rejoin with the invite code.`,
    );
    if (!confirmed) return;
    setLeaving(true);
    await supabase
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--black)" }}>
      ...
      <div
        style={{ maxWidth: 860, margin: "0 auto", padding: "88px 24px 60px" }}
      >
        {/* Back */}
        <Link
          href="/groups"
            ...
            >
              <div>
                ...
                >
                  ...
                  {dynamicScores.length === 1 ? "member" : "members"}
                </p>
              </div>
        ...
        {myEntry && (
          ...
          >
            {[
              ...
              {
                label: "Points",
                value: myEntry.display_points,
                accent: "var(--green-bright)",
              },
              {
                label: "Exact scores",
                value: myEntry.exact_scores,
                accent: "var(--blue-accent)",
              },
              {
                label: "Streak",
                value: `${myEntry.streak} 🔥`,
                accent: "#e05c4a",
              },
            ].map((s) => (
              <div
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    >
                      ...
                      <span>
                        {s.dynamic_correct ?? s.correct_results} correct
                      </span>
                      {(s.dynamic_streak || 0) > 0 && (
                        <span style={{ color: "#e05c4a" }}>
                          🔥 {s.dynamic_streak} streak
                        </span>
                      )}
                    </div>
                  </div>
