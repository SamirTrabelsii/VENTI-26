...
const EMPTY_GROUP_STANDINGS = GROUPS.reduce(
  (acc, group) => {
...
function computeRealState(matches: DbMatch[]): RealTournamentState {
  ...
  GROUPS.forEach((group) => {
    ...
    const isGroupComplete =
      dbGroupMatches.length === groupMatches.length &&
      dbGroupMatches.every(
        (m) =>
          m.status === "finished" &&
  ...
  matches.forEach((match) => {
    ...
    if (match.home_score > match.away_score)
      knockoutResults[key] = match.home_team;
    else if (match.away_score > match.home_score)
      knockoutResults[key] = match.away_team;
  ...
  useEffect(() => {
    ...
    const loadRealState = async () => {
      const { data } = await supabase
        .from("matches")
        .select(
          "id, group_label, home_team, away_team, home_score, away_score, status, qualifier",
        );
