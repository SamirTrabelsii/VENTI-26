...
export const BADGE_DEFINITIONS: BadgeDefinition[] = [
    { id: 'completionist', icon: '🥉', label: 'Completionist', description: 'Predict every World Cup match before the tournament kickoff.', tier: 'bronze' },
    { id: 'sharpshooter', icon: '🥈', label: 'Sharpshooter', description: 'Predict 5 exact scores.', tier: 'silver' },
    { id: 'match_predictor', icon: '🥈', label: 'Match Predictor', description: 'Predict 25 match outcomes correctly.', tier: 'silver' },
    { id: 'hot_streak', icon: '🥈', label: 'Hot Streak', description: 'Predict 5 consecutive match outcomes correctly.', tier: 'silver' },
    { id: 'group_master', icon: '🥇', label: 'Group Master', description: 'Correctly predict all matches of a single World Cup group.', tier: 'gold' },
    { id: 'lone_wolf', icon: '🥇', label: 'Lone Wolf', description: 'Be the only player to predict the exact score of a match.', tier: 'gold' },
    { id: 'consistency_king', icon: '🥇', label: 'Consistency King', description: 'Maintain a prediction accuracy above 50% after finishing the groups phase.', tier: 'gold' },
    { id: 'deadly_accuracy', icon: '💎', label: 'Deadly Accuracy', description: 'Predict 3 exact scores in 3 consecutive match slots.', tier: 'diamond' },
    { id: 'r32_oracle', icon: '💎', label: 'Round of 32 Oracle', description: 'Correctly predict all teams qualifying for the Round of 32 from original prediction.', tier: 'diamond' },
    { id: 'time_traveler', icon: '💎', label: 'Time Traveler', description: 'Correctly predict both finalists and the World Cup winner before kickoff.', tier: 'diamond' },
    { id: 'final_visionary', icon: '👑', label: 'Final Visionary', description: 'Correctly predict both finalists from original prediction.', tier: 'crown' },
    { id: 'world_champion', icon: '👑', label: 'World Champion', description: 'Correctly predict the World Cup winner from original prediction.', tier: 'crown' },
    { id: 'knockout_master', icon: '👑', label: 'Knockout Master', description: 'Correctly predict every knockout-stage match using the live bracket.', tier: 'crown' },
    { id: 'perfect_bracket', icon: '⚡', label: 'Perfect Bracket', description: 'Correctly predict every knockout-stage match from original prediction.', tier: 'lightning' },
]
...
export function evaluateBadges(ctx: EvalContext): Set<string> {
    ...
    for (const match of ctx.finishedMatches) {
        ...
        const isCorrectOutcome = outcome(pred.home_score, pred.away_score) === outcome(match.home_score, match.away_score)
    ...
    const correctOutcomeCount = matchResults.filter(r => r.isCorrectOutcome).length
    ...
    for (const g of groups) {
        ...
        const finishedGroupMatches = ctx.finishedMatches.filter(m => m.group_label === g)
    ...
    for (const match of ctx.finishedMatches) {
        ...
        const exactPreds = ctx.allPredictions.filter(p =>
            p.match_id === match.id &&
    ...
    const groupStageFinished = ctx.finishedMatches.filter(m => !isKnockoutMatch(m))
    if (groupStageFinished.length >= GROUP_STAGE_MATCHES) {
        const groupResults = matchResults.filter(r => !isKnockoutMatch({ group_label: r.groupLabel }))
    ...
    for (let i = 0; i <= matchResults.length - 3; i++) {
        if (matchResults[i].isExact && matchResults[i + 1].isExact && matchResults[i + 2].isExact) {
    ...
    if (championPick && finalMatch && finalMatch.home_score !== null) {
        ...
        const actualWinner = finalMatch.home_score > finalMatch.away_score
    ...
    if (finalMatch && finalPicks.length >= 2) {
        ...
        const actualFinalists = new Set([finalMatch.home_team, finalMatch.away_team])
    ...
    const koMatches = matchResults.filter(r => isKnockoutMatch({ group_label: r.groupLabel }))
    const finishedKoMatches = ctx.finishedMatches.filter(m => isKnockoutMatch(m))
 ...
 */
export function getBadgeProgress(ctx: Omit<EvalContext, 'allPredictions'>, earned: Set<string>): Record<string, { current: number; target: number }> {
    const progress: Record<string, { current: number; target: number }> = {}

    // Completionist
    progress.completionist = { current: ctx.predictions.length + ctx.bracketPicks.length, target: TOTAL_MATCHES }

    // Build match results
    const matchResults: Array<{ isExact: boolean; isCorrectOutcome: boolean; groupLabel: string }> = []
    for (const match of ctx.finishedMatches) {
        const pred = ctx.predictions.find(p => p.match_id === match.id)
        if (!pred || typeof pred.home_score !== 'number') {
            matchResults.push({ isExact: false, isCorrectOutcome: false, groupLabel: match.group_label })
            continue
        }
        const ko = isKnockoutMatch(match)
        const result = scoreMatch(pred.home_score, pred.away_score, match.home_score, match.away_score, ko)
        const isCorrectOutcome = outcome(pred.home_score, pred.away_score) === outcome(match.home_score, match.away_score)
        matchResults.push({ isExact: result.type === 'exact', isCorrectOutcome, groupLabel: match.group_label })
    }
    ...
    for (const r of matchResults) {
        if (r.isCorrectOutcome) { streak++; if (streak > maxStreak) maxStreak = streak }
        else streak = 0
    }
    ...
    const groupResults = matchResults.filter(r => !isKnockoutMatch({ group_label: r.groupLabel }))
    const groupCorrect = groupResults.filter(r => r.isCorrectOutcome).length
    const accuracy = groupResults.length > 0 ? Math.round((groupCorrect / groupResults.length) * 100) : 0
    progress.consistency_king = { current: accuracy, target: 50 }

    // Deadly Accuracy - best consecutive exact run
    let maxExactRun = 0, exactRun = 0
    for (const r of matchResults) {
        if (r.isExact) { exactRun++; if (exactRun > maxExactRun) maxExactRun = exactRun }
        else exactRun = 0
    }
