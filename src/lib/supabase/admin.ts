...
export function createAdminClient() {
    ...
    if (!url || !key) {
        throw new Error('Missing NEXT_PUBLIC_SUPABASE_URL or VITE_SUPABASE_SECRET_ROLE_KEY')
    }
...
export async function verifyAdmin() {
    ...
    if (!profile || profile.display_name.toLowerCase() !== adminUsername.toLowerCase()) return null
