...
export const NAME_TO_CODE: Record<string, string> = {
    ...
    'Australia': 'AUS', 'Turkey': 'TUR', 'Germany': 'GER', 'Curaçao': 'CUW', 'Curacao': 'CUW',
    'Netherlands': 'NED', 'Japan': 'JPN', 'Ivory Coast': 'CIV', "Côte d'Ivoire": 'CIV',
    "CÃ´te d'Ivoire": 'CIV', 'Ecuador': 'ECU', 'Sweden': 'SWE', 'Tunisia': 'TUN',
...
export function roundSlotFromFixtureId(id: string): { round: KnockoutRound; slotIndex: number } {
...
function matchKnownApiByOfficialNumber(apiMatches: LiveApiMatch[], matchNumber: number) {
    return apiMatches.find(match => String(match.id) === String(matchNumber))
}

export function buildLiveBracketFixtures(apiMatches: LiveApiMatch[], dbMatches: any[] = []): LiveBracketFixture[] {
    const apiByLocalId = new Map<string, LiveApiMatch>()

    for (const groupLabel of ['R32', 'R16', 'QF', 'SF', '3RD', 'FINAL']) {
        const localFixtures = KNOCKOUT_MATCHES.filter(match => match.group_label === groupLabel)
        ...
        localFixtures.forEach((fixture, index) => {
            const apiByNumber = matchKnownApiByOfficialNumber(apiMatches, fixture.match_number)
    ...
    return KNOCKOUT_MATCHES.map(fixture => {
        ...
        const homeCode = isKnownTeamCode(homeFromApi) ? homeFromApi : isKnownTeamCode(homeFromDb) ? homeFromDb : 'TBD'
        const awayCode = isKnownTeamCode(awayFromApi) ? awayFromApi : isKnownTeamCode(awayFromDb) ? awayFromDb : 'TBD'
        ...
        return {
            ...
            source: isKnownTeamCode(homeFromApi) || isKnownTeamCode(awayFromApi) ? 'api' : isKnownTeamCode(homeFromDb) || isKnownTeamCode(awayFromDb) ? 'db' : 'template',
...
export function buildFinishedKnockoutWinners(fixtures: LiveBracketFixture[]) {
    ...
    for (const fixture of fixtures) {
        ...
        if (!isKnownTeamCode(fixture.homeCode) || !isKnownTeamCode(fixture.awayCode)) continue
        if (fixture.homeScore > fixture.awayScore) winners[fixture.id] = fixture.homeCode
        if (fixture.awayScore > fixture.homeScore) winners[fixture.id] = fixture.awayCode
    }
    return winners
}
...
export function buildGroupFinishedCount(apiMatches: LiveApiMatch[]) {
    ...
    for (const apiMatch of apiMatches) {
        if (apiMatch.stage !== 'GROUP_STAGE' || apiMatch.status !== 'FINISHED') continue
        const homeCode = apiCode(apiMatch.homeTeam)
        const awayCode = apiCode(apiMatch.awayTeam)
        const local = GROUP_MATCHES.find(match => match.home_team === homeCode && match.away_team === awayCode)
